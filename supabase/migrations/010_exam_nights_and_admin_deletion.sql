create or replace function public.study_mode_for_date(p_date date, p_exam_start date, p_exam_end date)
returns text
language sql
immutable
...
as $$
  select case
    when p_exam_start is null or p_exam_end is null then 'normal'
    when p_date between p_exam_start - 1 and p_exam_end - 1 then 'exam'
    when p_date between p_exam_start - 14 and p_exam_start - 2 then 'pre_exam'
    else 'normal'
  end
$$;

revoke all on function public.study_mode_for_date(date, date, date) from public, anon, authenticated;
...
begin
  ...
  select * into v_settings from public.system_settings where id=1;
  v_session2_available:=public.study_mode_for_date(v_date,v_settings.exam_start,v_settings.exam_end)='exam';
...
as $fn$
declare
  v_user_id uuid:=auth.uid(); v_now timestamp:=timezone('Asia/Seoul',now()); v_current_date date;
  v_settings public.system_settings%rowtype; v_editable boolean; v_attendance_open boolean; v_session2_available boolean;
begin
  if not exists(select 1 from public.profiles where id=v_user_id and role in ('admin','super_admin')) then raise exception '관리자 권한이 필요합니다.'; end if;
  ...
  v_session2_available:=not v_editable or public.study_mode_for_date(p_date,v_settings.exam_start,v_settings.exam_end)='exam';
  return jsonb_build_object('date',to_char(p_date,'YYYY-MM-DD'),'currentDate',to_char(v_current_date,'YYYY-MM-DD'),
    'session',p_session,'session2Available',v_session2_available,'attendanceOpen',v_attendance_open,'editable',v_editable,
    'floors',(select jsonb_agg(jsonb_build_object('floor',f.floor,'gender',case when f.floor<=4 then 'male' else 'female' end,
      'seats',(select jsonb_agg(jsonb_build_object('number',s.seat,'occupied',r.id is not null,'reservationId',r.id,
        'applicantName',p.name,'applicantRoom',p.room,'attendanceStatus',a.status,'absenceCount',p.absence_count,
        'bannedUntil',to_char(case when p.banned_until>p_date then p.banned_until else null end,'YYYY-MM-DD')) order by s.seat)
        from generate_series(1,public.floor_seat_count(f.floor)) s(seat)
        left join public.reservations r on r.study_date=p_date and r.session=p_session and r.floor=f.floor and r.seat=s.seat
        left join public.profiles p on p.id=r.user_id
        left join public.attendance_records a on a.study_date=p_date and a.user_id=r.user_id)) order by f.floor)
      from generate_series(1,8) f(floor)));
end;
$fn$;
...
alter table public.admin_invites drop constraint if exists admin_invites_used_by_fkey;
alter table public.admin_invites add constraint admin_invites_used_by_fkey
...
alter table public.attendance_records add constraint attendance_records_marked_by_fkey
  foreign key (marked_by) references public.profiles(id) on delete set null;

create or replace function public.delete_admin_account(p_target uuid)
returns void language plpgsql security definer set search_path = public, auth
as $fn$
declare v_user_id uuid:=auth.uid();
begin
  if not exists(select 1 from public.profiles where id=v_user_id and role='super_admin') then
    raise exception '총관리자 권한이 필요합니다.';
  end if;
  perform 1 from public.profiles where id=p_target and role='admin' for update;
  if not found then raise exception '삭제할 일반 관리자 계정을 찾을 수 없습니다.'; end if;
